...
class TestDevOpsEngineerPRReminderWorkflow:
    ...
    def test_debug_why_pr_details_not_called(self):
        """
        Debug test to understand why PR details are not being fetched
        """
        ...
        with (
            patch(
                "pdp_dev_mcp.tools.common.repository_context.get_repository_context"
            ) as mock_get_context,
            patch(
                "pdp_dev_mcp.tools.common.azure_devops_pr_review.subprocess.run"
            ) as mock_subprocess,
        ):
    ...
    def test_engineer_gets_clear_error_when_repository_context_missing(self):
        ...
        with patch(
            ...
            assert result.get("suggestion"), (
                "Suggestion should be provided when repository context fails"
            )
